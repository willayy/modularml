 ...
 */
template <typename T>
Tensor<T> generate_random_tensor(const array_mml<size_t> &shape, T lo_v = T(0),
                                 T hi_v = T(1));
...
#define _TENSOR_UTILS(DT)                                                      \
  template bool TensorUtils::tensors_are_close<DT>(Tensor<DT> &, Tensor<DT> &, \
                                                   DT);                        \
  template Tensor<DT> TensorUtils::generate_random_tensor<DT>(                 \
      const array_mml<size_t> &, DT, DT);

#define _TENSOR_UTILS_REAL(DT)                                                \
  template void TensorUtils::kaiming_uniform<DT>(                             \
      std::shared_ptr<Tensor<DT>>, size_t, size_t, std::mt19937 &);           \
  template void TensorUtils::kaiming_uniform<DT>(std::shared_ptr<Tensor<DT>>, \
                                                 size_t, size_t);
