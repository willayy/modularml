...
MML_DataStructure<T>::MML_DataStructure(const vec<int>& shape,
  ...
  if (data_size != data.size())
    throw std::invalid_argument(
        "The size of the data and the shape does not match");
...
T MML_DataStructure<T>::get(const vec<int>& indices) const {
  ...
  if (indices.size() != this->shape.size())
    throw std::invalid_argument(
        "The diemensions of the indices does not match with the data "
        "structure");
...
void MML_DataStructure<T>::set_data(const vec<T> new_data) {
  ...
  if (this->get_size() != data.size())
    throw std::invalid_argument(
        "The size of the data provided mismatch with the size of the data "
        "structure");
...
int MML_DataStructure<T>::get_size() const {
  return this->data
      .size();  // This assumes that the structure is populated by data
}

template <typename T>
bool MML_DataStructure<T>::equals(const MML_DataStructure<T>& other) const {
  if (this->data.size() ==
      other->data.size())  // if the number of diemensions are not the same then
                           // they can't be equal
  {
    for (int i = 0; i < this->data.size(); ++i) {
      if (this->data.at(i) != other->data.size())
        return false;  // Check the diemensions of the structures
    }
    return true;
  }
  return false;
...
int MML_DataStructure<T>::calc_offset(const vec<int>& indices) const {
  int offset = 0;
  int factor = 1;
  for (int i = indices.size() - 1; i >= 0; --i)  // Iterates backwards
  {
    if (indices.at(i) < 0 ||
        indices.at(i) >=
            this->shape.at(i))  // Check that the index isn't out of range
    {
      throw std::out_of_range("Index in indices vector are out of range " +
                              std::to_string(i));
    }
