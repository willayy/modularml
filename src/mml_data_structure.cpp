...
const string MML_DataStructure<T>::get_shape_str() const
{
    ...
    {
        if (i + 1 == this->shape.size()) result += std::to_string(i);
...
{
    if (this->data.size() == other->data.size()) // if the number of diemensions are not the same then they can't be equal
    {
        for (int i = 0; i < this->data.size(); ++i)
        {
            if (this->data.at(i) != other->data.size()) return false; // Check the diemensions of the structures 
        }
        return true;
    }
    return false;
