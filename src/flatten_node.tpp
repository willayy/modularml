...
template <typename T> void FlattenNode<T>::forward() {
  ...
  if (get_axis() == 0) {
    // This gives a warning, but when get_size() returns int in the future it
    // will disappear
...
template <typename T> bool FlattenNode<T>::areInputsFilled() const {
...
template <typename T> bool FlattenNode<T>::areOutputsFilled() const {
...
template <typename T> uli FlattenNode<T>::get_axis() const { return axis; }