...
std::vector<std::vector<uint8_t>> load_mnist_images(std::string path) {
  ...
  std::vector<std::vector<uint8_t>> images(
      num_images, std::vector<uint8_t>(num_rows * num_cols));
 ...
 */
std::vector<int> run_lenet_inference(const std::unique_ptr<Model>& model,
...
int main(int argc, char* argv[]) {
  Profiler::begin_timing("Total runtime");
  ...
  for (int i = 0; i < num_validations; i++) {
    ...
    if (i % 10 == 0) {
      std::cout << "Processed " << i << " / " << num_validations << " images..."
                << std::endl;
    }
  }
  Profiler::end_timing("total inference");
  ...
  Profiler::end_timing("Total runtime");
  return 0;
}