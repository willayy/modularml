...
ConvNode<T>::ConvNode(shared_ptr<AbstractTensor> X,
                      ...
                      shared_ptr<AbstractTensor> Y, array_mml<uli> dilations,
                      array_mml<uli> padding, array_mml<uli> kernel_shape,
                      array_mml<uli> stride,
                      optional<shared_ptr<AbstractTensor>> B, uli group)
...
template <typename T> void ConvNode<T>::forward() {
  validate_inputs();
  ...
  auto im2col_output_shape = array_mml<uli>(
  ...
  array_mml<uli> result_shape(
      {W->get_shape()[0], im2col_output->get_shape()[1]});
  ...
  gemm->gemm_inner_product(
      0, 0, W->get_shape()[0], im2col_output->get_shape()[1], W->get_shape()[1],
      1.0f, W, W->get_shape()[1], im2col_output, im2col_output->get_shape()[1],
      0.0f, result_ptr, result_ptr->get_shape()[1]);

  // Reshape the flattened result
  result_ptr->reshape({get_batch_size(), get_out_channels(), get_out_height(),
                       get_out_width()});
...
template <typename T> bool ConvNode<T>::areInputsFilled() const {
  return X && X->get_size() > 0 && W && W->get_size() > 0 &&
...
template <typename T> bool ConvNode<T>::areOutputsFilled() const {
...
template <typename T> void ConvNode<T>::flip_kernel() {
...
template <typename T>
void ConvNode<T>::im2col(shared_ptr<Tensor<T>> input,
  ...
  for (uli n = 0; n < get_batch_size(); ++n) {
    for (uli h = 0; h < get_out_height(); ++h) {
      for (uli w = 0; w < get_out_width(); ++w) { // Traverse into each batch
        ...
        for (uli c = 0; c < get_in_channels();
             ++c) { // If the input has multiple channels, iterate over each one

          // Here we loop over the kernel's height and width, simulating how the
          // kernel moves across the input tensor. For each position of the
          // kernel, the corresponding input values are extracted and stored in
          // the output tensor. If the kernel extends beyond the boundaries of
          // the input (due to padding or stride), zero padding is added instead
          // of the input values.
          for (uli kh = 0; kh < get_kernel_height(); ++kh) {
            for (uli kw = 0; kw < get_kernel_width(); ++kw) {
              ...
              if (input_h < 0 ||
                ...
                uli row_index = c * get_kernel_height() * get_kernel_width() +
                ...
                uli input_index =
                    n * (get_in_channels() * get_in_height() * get_in_width()) +
                    c * (get_in_height() * get_in_width()) +
                    input_h * get_in_width() + input_w;

                // Check if input index is valid
                if (input_index >= 0 && input_index < get_in_channels() *
                                                          get_in_height() *
                                                          get_in_width()) {
    ...
    for (uli i = 0; i < get_out_channels(); ++i) {
      for (uli h = 0; h < get_out_height(); ++h) {
        for (uli w = 0; w < get_out_width(); ++w) {
          uli index = ((b * get_out_channels() + i) * get_out_height() + h) *
                          get_out_width() +
                      w;

          // Each value in bias vector is added to one entire out feature at a
          // time
...
template <typename T> void ConvNode<T>::validate_inputs() {
  if (!areInputsFilled())
    throw runtime_error("ConvNode inputs are not fully set.");

  auto x_shape = X->get_shape();
  if (x_shape.size() != 4)
    throw runtime_error("Input tensor must have 4 dimensions: (Features x "
                        "Channels x Height x Width)");

  if (dilations.size() != 2) {
    throw invalid_argument(
        "Invalid dilations size. Expected a vector of size 2, but got: " +
        std::to_string(dilations.size()) + ".");
  }

  if (padding.size() != 4) {
    throw invalid_argument(
        "Invalid padding vector size. Expected a vector of size 4, but got: " +
        std::to_string(padding.size()) + ".");
  }

  if (kernel_shape.size() != 2) {
    throw invalid_argument("Invalid kernel_shape vector size. Expected a "
                           "vector of size 2, but got: " +
                           std::to_string(kernel_shape.size()) + ".");
  }

  if (stride.size() != 2) {
    throw invalid_argument(
        "Invalid stride vector size. Expected a vector of size 2, but got: " +
        std::to_string(stride.size()) + ".");
  }
}

template <typename T> uli ConvNode<T>::get_batch_size() const {
  return batch_size;
}

template <typename T> uli ConvNode<T>::get_in_channels() const {
  return in_channels;
}

template <typename T> uli ConvNode<T>::get_in_height() const {
  return in_height;
}

template <typename T> uli ConvNode<T>::get_in_width() const { return in_width; }

template <typename T> uli ConvNode<T>::get_kernel_height() const {
  return kernel_height;
}

template <typename T> uli ConvNode<T>::get_kernel_width() const {
  return kernel_width;
}

template <typename T> uli ConvNode<T>::get_out_channels() const {
  return out_channels;
}

template <typename T> uli ConvNode<T>::get_stride_height() const {
  return stride[0];
}

template <typename T> uli ConvNode<T>::get_stride_width() const {
  return stride[1];
}

template <typename T> uli ConvNode<T>::get_padding_top() const {
  return padding[0];
}

template <typename T> uli ConvNode<T>::get_padding_bottom() const {
  return padding[1];
}

template <typename T> uli ConvNode<T>::get_padding_left() const {
  return padding[2];
}

template <typename T> uli ConvNode<T>::get_padding_right() const {
  return padding[3];
}

template <typename T> uli ConvNode<T>::get_out_height() {
...
template <typename T> uli ConvNode<T>::get_out_width() {
