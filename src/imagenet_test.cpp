...
std::unordered_map<std::string, int> load_labels(const std::string& json_file, const std::string& label_key) {
...
std::vector<int> run_alexnet_inference(const std::unique_ptr<Model>& model, std::string image_path) {
    ...
    std::vector<int> max_indices = TensorOperations::top_n_arg_max<float>(output_tensor, 5);
...
int main(int argc, char* argv[]) {
    ...
    std::string validation_labels = "demo/alexnet_demo/ILSVRC2012_validation_ground_truth.json";
    std::unordered_map<std::string, int> image_name_to_index = load_labels(validation_labels, "LABEL_KERAS-CAFFE");
    ...
    Profiler::end_timing("AlexNet testing");
