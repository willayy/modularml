...
GeluNode::GeluNode(const json& node) {
...
void GeluNode::forward(std::unordered_map<std::string, GeneralDataTypes>& iomap) {
  ...
  std::visit([&](const auto& x_ptr) {
    ...
    if constexpr (!is_in_variant_v<ValueTypeX, T>) {
      throw std::runtime_error("GELUNode: Unsupported data type for tensor X");
    } else {
      ...
      if (y_it == iomap.end()) {
        ...
        throw std::runtime_error("GELUNode: Output tensor Y has incorrect type");
      }
      ...
      if (approximate == "none") {
        arithmetic.elementwise(x_ptr, [](ValueTypeX val) -> ValueTypeX { 
          return static_cast<ValueTypeX>(0.5f * val * (1.0f + std::erf(val / std::sqrt(2.0f))));
        }, y_ptr);
      } else {
        arithmetic.elementwise(x_ptr, [](ValueTypeX val) -> ValueTypeX {
          return static_cast<ValueTypeX>(0.5f * val * (1.0f + std::tanh(std::sqrt(2.0f / M_PI) * (val + 0.044715f * std::pow(val, 3.0f)))));
        }, y_ptr);
      }
    }
  }, x_tensor);
...
std::vector<std::string> GeluNode::getInputs() {
  return {X};
}

std::vector<std::string> GeluNode::getOutputs() {
  return {Y};
}