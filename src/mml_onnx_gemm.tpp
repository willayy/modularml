...
shared_ptr<Tensor<T>> OnnxGemm_mml<T>::gemm_inner_product(shared_ptr<Tensor<T>> A, shared_ptr<Tensor<T>> B,
  ...
  const unsigned long int M = shape_A[0];
  const unsigned long int N = shape_B[1];
  const unsigned long int K = shape_A[1];
  const unsigned long int lda = K;
  const unsigned long int ldb = N;
  const unsigned long int ldc = N;
