...
LeakyReLUNode::LeakyReLUNode(const json& node) {
...
void LeakyReLUNode::forward(std::unordered_map<std::string, GeneralDataTypes>& iomap) {
  auto x_it = iomap.find(X);
  if (x_it == iomap.end()) {
    throw std::runtime_error("LeakyReLUNode: Input tensor X not found in iomap");
  }
  ...
  std::visit([&](const auto& x_ptr) {
    ...
    if constexpr (!is_in_variant_v<ValueTypeX, T>) {
      throw std::runtime_error("LeakyReLUNode: Unsupported data type for tensor X");
    } else {
      ...
      if (y_it == iomap.end()) {
        ...
        throw std::runtime_error("LeakyReLUNode: Output tensor Y has incorrect type");
      }
...
std::vector<std::string> LeakyReLUNode::getInputs() {
  return {X};
}

std::vector<std::string> LeakyReLUNode::getOutputs() {
  return {Y};
}