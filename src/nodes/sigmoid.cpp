...
SigmoidNode::SigmoidNode(const json& node) {
...
void SigmoidNode::forward(std::unordered_map<std::string, GeneralDataTypes>& iomap) {
  ...
  std::visit([&](const auto& x_ptr) {
    ...
    if constexpr (!is_in_variant_v<ValueTypeX, T>) {
      throw std::runtime_error("SigmoidNode: Unsupported data type for tensor X");
    } else {
      ...
      if (y_it == iomap.end()) {
        ...
        throw std::runtime_error("SigmoidNode: Output tensor Y has incorrect type");
      }

      auto y_ptr = std::get<std::shared_ptr<Tensor<ValueTypeX>>>(y_it->second);

      Arithmetic_mml<ValueTypeX> arithmetic;
      arithmetic.elementwise(x_ptr, [](ValueTypeX x) -> ValueTypeX { 
        return 1 / (1 + exp(-x)); 
      }, y_ptr);
    }
  }, x_tensor);
...
std::vector<std::string> SigmoidNode::getInputs() {
  return {X};
}

std::vector<std::string> SigmoidNode::getOutputs() {
  return {Y};
}
