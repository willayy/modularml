...
PoolingNode_mml::PoolingNode_mml(std::string input,
                                 std::vector<std::string> outputs,
                                 array_mml<size_t> kernel_shape,
                                 array_mml<size_t> strides,
                                 std::string auto_pad, size_t ceil_mode,
                                 array_mml<size_t> dilations,
                                 array_mml<size_t> pads)
    : input(input), outputs(outputs), kernel_shape(kernel_shape),
...
PoolingNode_mml::PoolingNode_mml(const nlohmann::json &node) {
  ...
  if (node.contains("attribute") && node["attribute"].is_array()) {
    for (const auto &attr : node["attribute"]) {
      if (attr["name"] == "kernel_shape") {
        ...
        kernel_shape = array_mml<size_t>(values);
      } else if (attr["name"] == "strides") {
        std::vector<size_t> values;
        for (const auto &val : attr["ints"]) {
          values.push_back(std::stoul(val.get<std::string>()));
        }
        strides = array_mml<size_t>(values);
        ...
        dilations = array_mml<size_t>(values);
      } else if (attr["name"] == "pads") {
        std::vector<size_t> values;
        for (const auto &val : attr["ints"]) {
          values.push_back(std::stoul(val.get<std::string>()));
        }
        pads = array_mml<size_t>(values);
      }
    }
  }
}

void PoolingNode_mml::forward(
    std::unordered_map<std::string, GeneralDataTypes> &iomap) {
  auto input_it = iomap.find(input);
  if (input_it == iomap.end()) {
    throw std::runtime_error(
        "PoolingNode_mml: Input tensor not found in iomap");
  }
  ...
  std::visit(
      [&](const auto &input_ptr) {
        using ValueType = typename std::decay_t<
            decltype(input_ptr)>::element_type::value_type;

        if constexpr (!is_in_variant_v<ValueType, T>) {
          throw std::runtime_error(
              "PoolingNode_mml: Unsupported data type for tensor X");
        } else {
          array_mml<size_t> input_shape = input_ptr->get_shape();
          ...
          array_mml<size_t> effective_kernel_shape = array_mml(
              {kernel_shape[0] + (kernel_shape[0] - 1) * (dilations[0] - 1),
               kernel_shape[1] + (kernel_shape[1] - 1) * (dilations[1] - 1)});

          std::vector<size_t> pad_shape = {pads[0] + pads[1],
                                           pads[2] + pads[3]};
          ...
          for (size_t i = 2; i < 4; i++) {
            if (auto_pad == "VALID") {
              if (ceil_mode) {
                output_shape[i] = static_cast<size_t>(ceil(
              ...
              if (ceil_mode) {
                ...
                output_shape[i] = static_cast<size_t>(
                    ceil(static_cast<float>(input_shape[i]) / strides[i - 2]));

              } else {
                output_shape[i] = static_cast<size_t>(floor(
                                      (static_cast<float>(input_shape[i]) - 1) /
                                      static_cast<float>(strides[i - 2]))) +
                                  1;
              }
              pad_shape[i - 2] =
              ...
              if (ceil_mode) {
                output_shape[i] = static_cast<size_t>(ceil(
                    (static_cast<float>(input_shape[i]) + pad_shape[i - 2] -
                     dilations[i - 2] * (effective_kernel_shape[i - 2] - 1) -
                     1) /
                        strides[i - 2] +
                    1));
              } else {
