...
template <typename T>
static TensorOperationsModule<T>& TensorOperationsModule<T>::getInstance() {
    static TensorOperationsModule instance;
    return instance;
}

template <typename T>
void TensorOperationsModule<T>::set_operation_func(string id, function<void()> func) {
        if (id == "gemm") {
            static_assert(
...
template <typename T>
void TensorOperationsModule<T>::add(const shared_ptr<const Tensor<T>> a, const shared_ptr<const Tensor<T>> b, shared_ptr<Tensor<T>> c) const {
    this.add_ptr(a, b, c);
}

template <typename T>
void TensorOperationsModule<T>::subtract(const shared_ptr<Tensor<T>> a, const shared_ptr<Tensor<T>> b, shared_ptr<Tensor<T>> c) const {
    this.subtract_ptr(a, b, c);
}

template <typename T>
void TensorOperationsModule<T>::multiply(const shared_ptr<Tensor<T>> a, const T b, shared_ptr<Tensor<T>> c) const {
    this.multiply_ptr(a, b, c);
}

template <typename T>
bool TensorOperationsModule<T>::equals(const shared_ptr<Tensor<T>> a, const shared_ptr<Tensor<T>> b) const {
    return this.equals_ptr(a, b);
}

template <typename T>
void TensorOperationsModule<T>::elementwise(const shared_ptr<const Tensor<T>> a, T (*f)(T), const shared_ptr<Tensor<T>> c) const {
    this.elementwise_ptr(a, f, c);
}

template <typename T>
void TensorOperationsModule<T>::elementwise_in_place(const shared_ptr<Tensor<T>> a, T (*f)(T)) const {
    this.elementwise_in_place_ptr(a, f);
}

template <typename T>
void TensorOperationsModule<T>::gemm(int TA, int TB, int M, int N, int K, T ALPHA,
...
template <typename T>
shared_ptr<Tensor<T>> TensorOperationsModule<T>::gemm_onnx(shared_ptr<Tensor<T>> A, shared_ptr<Tensor<T>> B,
