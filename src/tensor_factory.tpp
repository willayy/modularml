...
template <typename T>
static TensorFactory<T>& TensorFactory<T>::getInstance() {
    static TensorFactory instance;
    return instance;
}

template <typename T>
shared_ptr<Tensor<T>> TensorFactory<T>::create_tensor(const array_mml<uli> &shape, const array_mml<T> &data) const {
    return this->tensor_constructor_1(shape, data);
}

template <typename T>
shared_ptr<Tensor<T>> TensorFactory<T>::create_tensor(const array_mml<uli> &shape) const {
    return this->tensor_constructor_2(shape);
}

template <typename T>
shared_ptr<Tensor<T>> TensorFactory<T>::create_tensor(const initializer_list<uli> shape, const initializer_list<T> data) const {
    return this->tensor_constructor_3(shape, data);
}

template <typename T>
shared_ptr<Tensor<T>> TensorFactory<T>::create_tensor(const initializer_list<uli> shape) const {
    return this->tensor_constructor_4(shape);
}

template <typename T>
shared_ptr<Tensor<T>> TensorFactory<T>::random_tensor(const array_mml<uli> &shape, T lo_v, T hi_v) const {
    uli n = 1;
...
template <typename T>
array_mml<T> TensorFactory<T>::random_array_mml_integral(uli lo_sz, uli hi_sz, T lo_v, T hi_v) const {
...
template <typename T>
array_mml<T> TensorFactory<T>::random_array_mml_real(uli lo_sz, uli hi_sz, T lo_v, T hi_v) const {
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<size_t> size_dist(lo_sz, hi_sz);
  uli n = size_dist(gen);
