...
void GemmNode::forward() {
    ...
    visit([this](auto &a_tensor) {
        using T = typename remove_reference_t<decltype(a_tensor)>::value_type;
        ...
        auto &b_tensor = get<Tensor_mml<T>>(*B);
        ...
        auto A_ptr = make_shared<Tensor_mml<T>>(a_tensor);
        auto B_ptr = make_shared<Tensor_mml<T>>(b_tensor);

        // Prepare C: if provided, wrap it; otherwise, create a zero tensor.
        shared_ptr<Tensor_mml<T>> C_ptr;
        if (C.has_value() && C.value()) {
            auto &c_tensor = get<Tensor_mml<T>>(*C.value());
            C_ptr = make_shared<Tensor_mml<T>>(c_tensor);
        } else {
