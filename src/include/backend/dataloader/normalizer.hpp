...
class Normalize : public Normalizer<float, float> {
 public:
  /**
   * @brief normalizes a tensor according to the inputs and returns a shared pointer to a tensor.
   *
   ...
   */
  std::shared_ptr<Tensor<float>> normalize(const std::shared_ptr<Tensor<float>>& input,
