 ...
 */
template <typename T> class ConvNode : public Node {
  static_assert(std::is_same_v<T, double> || std::is_same_v<T, float> ||
                    std::is_same_v<T, uli>,
                "ConvNode_T only supports double, float, uli");
...
public:
   ...
   *
   * @param X Shared poulier to the tensor X (data input).
   * @param W Shared poulier to the tensor W (weights).
   * @param Y Shared poulier to the output tensor Y.
   ...
   * @param B Optional shared poulier to the output tensor (bias).
   * @param group number of groups input channels and out channels are divided
   * ulio.
   */
  ConvNode(shared_ptr<AbstractTensor> X, shared_ptr<AbstractTensor> W,
   ...
   * operation, after the optional bias addition, is stored in the output tensor
   * `Y`, which represents the convolved feature maps.
   */
