...
template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_outer_product(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                                   float alpha = 1.0, float beta = 1.0,
                                                   int transA = 0, int transB = 0,
                                                   optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_row_wise_product(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                                      float alpha = 1.0, float beta = 1.0,
                                                      int transA = 0, int transB = 0,
                                                      optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_col_wise_product(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                                      float alpha = 1.0, float beta = 1.0,
                                                      int transA = 0, int transB = 0,
                                                      optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_blocked(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                             float alpha = 1.0, float beta = 1.0,
                                             int transA = 0, int transB = 0,
                                             optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_avx(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                         float alpha = 1.0, float beta = 1.0,
                                         int transA = 0, int transB = 0,
                                         optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_avx512(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                            float alpha = 1.0, float beta = 1.0,
                                            int transA = 0, int transB = 0,
                                            optional<shared_ptr<Tensor<T>>> C = nullopt);

template <typename T>
static shared_ptr<Tensor<T>> mml_onnx_gemm_intel_MKL(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                               float alpha = 1.0, float beta = 1.0,
                                               int transA = 0, int transB = 0,
                                               optional<shared_ptr<Tensor<T>>> C = nullopt);
...
template <typename T>
static void mml_elementwise(const shared_ptr<const Tensor<T>> a, T (*f)(T), const shared_ptr<Tensor<T>> c);

template <typename T>
static void mml_elementwise_in_place(const shared_ptr<Tensor<T>> a, T (*f)(T));