...
class AvgPoolingLayer : public PoolingLayer < GeneralDataTypes {
 public:
  AvgPoolingLayer(vector<int> f, vector<int> s, string p = "valid")
      : PoolingLayer<GeneralDataTypes>(f, s, p) {}

 private:
  GeneralDataTypes pooling(const shared_ptr<Tensor<GeneralDataTypes>> t, array_mml<int> shape, int element,
                           int channel, int in_row_start, int in_col_start) const override;
};

#include "../mml_avg_pooling_node.cpp"