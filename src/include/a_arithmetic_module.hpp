...
class ArithmeticModule {
 public:
  ...
  virtual std::shared_ptr<DataStructure<T>> add(const std::shared_ptr<DataStructure<T>> a, const std::shared_ptr<DataStructure<T>> b) const = 0;
  ...
  virtual std::shared_ptr<DataStructure<T>> subtract(const std::shared_ptr<DataStructure<T>> a, const std::shared_ptr<DataStructure<T>> b) const = 0;

  /// @brief Multiply two data structures.
  /// @param a The first data structure.
  /// @param b The second data structure.
  /// @return The result of multiplying the two data structures.
  virtual std::shared_ptr<DataStructure<T>> multiply(const std::shared_ptr<DataStructure<T>> a, const std::shared_ptr<DataStructure<T>> b) const = 0;
  ...
  virtual std::shared_ptr<DataStructure<T>> multiply(const std::shared_ptr<DataStructure<T>> a, const T b) const = 0;

  /// @brief Divide a data structure by a scalar.
  /// @param a The data structure.
  /// @param b The scalar value.
  /// @return The result of dividing the data structure by the scalar.
  virtual std::shared_ptr<DataStructure<T>> divide(const std::shared_ptr<DataStructure<T>> a, const T b) const = 0;

  /// @brief Check if two data structures are equal.
  /// @param a The first data structure.
  /// @param b The second data structure.
  /// @return True if the data structures are equal, false otherwise.
  virtual bool equals(const std::shared_ptr<DataStructure<T>> a, const std::shared_ptr<DataStructure<T>> b) const = 0;
