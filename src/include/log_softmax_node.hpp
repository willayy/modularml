 ...
 */
template <typename T> class LogSoftMaxNode : public Node {
  static_assert(
      std::is_same_v<T, float> || std::is_same_v<T, double>,
      "LogSoftMaxNode supports only float, double"); // bfloat16 and float16
                                                     // also but dont know how
                                                     // to include them
                                                     // currently
...
public:
   ...
   * (default -1)
   */
  LogSoftMaxNode(shared_ptr<const AbstractTensor> X,
                 shared_ptr<AbstractTensor> Y, uli axis = -1);
