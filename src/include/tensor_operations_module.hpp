...
/**
*   A module for performing arithmetic operations on tensor structures. Your modular toolbox for all operations on tensors.
...
class TensorOperationsModule {
 ...
 public:
   ...
   * @param ldc Specifies the first dimension of matrix C. */
   ...
   /**
   * @brief General matrix multiplication (GEMM) function using the ONNX standard.
   ...
   * @return Output tensor Y. Output tensor of shape (M, N). */
   static shared_ptr<Tensor<T>> gemm_onnx(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
    ...
    shared_ptr<Tensor<T>> (*gemm_onnx_ptr)(shared_ptr<Tensor<T>> A = nullptr, shared_ptr<Tensor<T>> B = nullptr,
                                              float alpha = 1.0, float beta = 1.0,
                                              int transA = 0, int transB = 0,
                                              optional<shared_ptr<Tensor<T>>> C = nullopt);
    // Pointer to the add function.
    void (*add_ptr)(const shared_ptr<const Tensor<T>> a, const shared_ptr<const Tensor<T>> b, shared_ptr<Tensor<T>> c);
    // Pointer to the subtract function.
    void (*subtract_ptr)(const shared_ptr<Tensor<T>> a, const shared_ptr<Tensor<T>> b, shared_ptr<Tensor<T>> c);
    // Pointer to the multiply function.
    void (*multiply_ptr)(const shared_ptr<Tensor<T>> a, const T b, shared_ptr<Tensor<T>> c);
    // Pointer to the equals function.
    bool (*equals_ptr)(const shared_ptr<Tensor<T>> a, const shared_ptr<Tensor<T>> b);
    // Pointer to the elementwise function.
    void (*elementwise_ptr)(const shared_ptr<const Tensor<T>> a, T (*f)(T), const shared_ptr<Tensor<T>> c);
