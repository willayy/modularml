...
TEST(test_mml_arithmetic, test_add_2) {
  ...
  const std::shared_ptr<Tensor<float>> b =
      std::make_shared<Tensor<float>>(array_mml<size_t>{3, 3}, array_mml<float>{1, 2, 3, 4, 5, 6, 7, 8, 9});
  const std::shared_ptr<Tensor<float>> c = std::make_shared<Tensor<float>>(array_mml<size_t>{3, 3});
  const std::shared_ptr<Tensor<float>> d =
      std::make_shared<Tensor<float>>(array_mml<size_t>{3, 3}, array_mml<float>{2, 4, 6, 8, 10, 12, 14, 16, 18});
  Arithmetic::add<float>(a, b, c);
  ASSERT_EQ((*c), (*d));
}
...
TEST(test_mml_arithmetic, test_div_1) {
  ...
  Arithmetic::multiply<float>(a, b, c);
  ASSERT_EQ((*c), (*d));
}
...
TEST(test_mml_arithmetic, test_div_2) {
  ...
  const std::shared_ptr<Tensor<float>> c = std::make_shared<Tensor<float>>(array_mml<size_t>{3, 3});
  const std::shared_ptr<Tensor<float>> d =
      std::make_shared<Tensor<float>>(array_mml<size_t>{3, 3}, array_mml<float>{0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5});
  Arithmetic::multiply<float>(a, b, c);
  ASSERT_EQ((*c), (*d));
}
...
TEST(test_mml_arithmetic, test_mul_1) {
  ...
  Arithmetic::subtract<float>(a, b, c);
  ASSERT_EQ((*c), (*d));
}
...
TEST(test_mml_arithmetic, test_elementwise) {
  ...
  Arithmetic::elementwise<float>(a, square, b);
  ASSERT_EQ(*b, *c);
}
...
TEST(test_mml_arithmetic, test_elementwise_in_place) {
  ...
  const std::shared_ptr<Tensor<float>> b = std::make_shared<Tensor<float>>(
    array_mml<size_t>{3, 3}, array_mml<float>{1.0f, 4.0f, 9.0f, 16.0f, 25.0f, 36.0f, 49.0f, 64.0f, 81.0f});
  Arithmetic::elementwise_in_place<float>(a, square);
  ASSERT_EQ(*a, *b);
}
...
TEST(test_mml_arithmetic, test_argmax_1) {
  const std::shared_ptr<Tensor<float>> a =
      std::make_shared<Tensor<float>>(array_mml<size_t>{2, 3}, array_mml<float>{1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f});
  const int b = 5;
  ASSERT_EQ(Arithmetic::arg_max<float>(a), b);
}

TEST(test_mml_arithmetic, test_argmax_2) {
  const std::shared_ptr<Tensor<float>> a = std::make_shared<Tensor<float>>(array_mml<size_t>{0});
  ASSERT_THROW(Arithmetic::arg_max<float>(a), std::runtime_error);
}
...
TEST(test_mml_arithmetic, test_argmax_3) {
  ...
  ASSERT_EQ(Arithmetic::arg_max<float>(a), expected_index);
}
