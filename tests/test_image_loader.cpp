  ...
  for (int c = 0; c < image_tensor->get_shape()[1]; c++) {
      ...
      for (int i = 0; i < image_tensor->get_shape()[2]; i++) {
          for (int j = 0; j < image_tensor->get_shape()[3]; j++) {
              int index = c * (image_tensor->get_shape()[2] *
  image_tensor->get_shape()[3]) + i * image_tensor->get_shape()[3] + j; float
  num = (*image_tensor)[index]; char cs = '.'; if (num > 0.2) cs = ';'; if (num
  > 0.4) cs = '>'; if (num > 0.6) cs = '%'; if (num > 0.8) cs = '#';
