...
const std::shared_ptr<OnnxGemmModule<float>> gm_f =
    std::make_shared<OnnxGemm_mml<float>>();
const std::shared_ptr<OnnxGemmModule<int>> gm_i =
    std::make_shared<OnnxGemm_mml<int>>();
...
TEST(test_mml_onnx_gemm, test_inner_product_1) {
  ...
  const std::shared_ptr<Tensor<float>> res =
      gm_f->gemm_inner_product(a, b, alpha, beta, 0, 0, c);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_inner_product_2) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res = gm_f->gemm_inner_product(a, b);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_outer_produt_1) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res =
      gm_f->gemm_outer_product(a, b, alpha, beta, 0, 0, c);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_outer_produt_2) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res = gm_f->gemm_outer_product(a, b);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_row_wise_product_1) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res =
      gm_f->gemm_row_wise_product(a, b, alpha, beta, 0, 0, c);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_row_wise_product_2) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res = gm_f->gemm_row_wise_product(a, b);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_col_wise_product_1) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res =
      gm_f->gemm_col_wise_product(a, b, alpha, beta, 0, 0, c);
  ASSERT_EQ((*res), (*d));
}
...
TEST(test_mml_onnx_gemm, test_col_wise_product_2) {
  ...
  const std::shared_ptr<Tensor<float>> d =
      TensorFactory::create_tensor<float>({2, 2}, {40, 46, 94, 109});
  const std::shared_ptr<Tensor<float>> res = gm_f->gemm_col_wise_product(a, b);
  ASSERT_EQ((*res), (*d));
}