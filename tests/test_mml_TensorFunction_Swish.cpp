...
TEST(test_mml_TensorFunction_Swish, test_SwishFunction_double) {
   ...
   * @brief Tensor to test and compare the operations on.
   */
  Tensor<double> t1 = tensor_mml<double>({3, 3}, {-1.0, 0.0, 1.0, -2.0, 2.0, -3.0, 3.0, 4.0, -4.0});

  /**
   * @brief Expected Tensor after the Swish function is applied to each element.
   */
  Tensor<double> expected_func = tensor_mml<double>({3, 3}, {-0.268941, 0.0, 0.731059, -0.238405, 1.761595, -0.142278, 2.857722, 3.928055, -0.071945});

  /**
   * @brief Expected Tensor after the derivative of the Swish function is applied
   * to each element.
   */
  Tensor<double> expected_derivative = tensor_mml<double>({3, 3}, {0.072329, 0.5, 0.927671, -0.090838, 1.090838, -0.088145, 1.088145, 1.052651, -0.052651});

  /**
   * @brief Expected Tensor after the approximation of the primitive of the Swish function is applied
   * to each element.
   */
  Tensor<double> expected_primitive = tensor_mml<double>({3, 3}, {1.044287, 0.693147, 1.044287, 1.888503, 1.888503, 2.906256, 2.906256, 3.946235, 3.946235});
