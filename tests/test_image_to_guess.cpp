...
TEST(test_alexnet_image_to_guess, image_to_guess) {
  ...
  const ImageLoaderConfig config(
  ...
  std::string temp_image_path =
      "../tests/data/alexnet/alexnet_pictures/temp_test_image.png";
  ...
  std::string class_name =
      get_class_name("../tests/data/alexnet/alexnet_ImageNet_labels.json",
                     std::to_string(result));
  std::cout << "Predicted class: " << class_name << std::endl;
  ASSERT_TRUE(class_name == "English_foxhound")
      << "Predicted class does not match expected class";
}
...
TEST(test_alexnet_image_to_guess, image_to_guess2) {
  ...
  std::string class_name =
      get_class_name("../tests/data/alexnet/alexnet_ImageNet_labels.json",
                     std::to_string(result));
  std::cout << "Predicted class: " << class_name << std::endl;
  ASSERT_TRUE(class_name == "American_egret")
      << "Predicted class does not match expected class";
}
