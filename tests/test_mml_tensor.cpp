...
TEST(test_mml_tensor, test_default_constructor_1) {
  ...
  auto expected_shape = array_mml<unsigned long int>({3, 3});
...
TEST(test_mml_tensor, test_move_assignment_2) {
  shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(array_mml<unsigned long int>({3, 3}));
  shared_ptr<Tensor<int>> t2 = make_shared<Tensor_mml<int>>(array_mml<unsigned long int>({2, 2}));
  *t2 = move(*t1);
  auto expected_shape = array_mml<unsigned long int>({3, 3});
  auto expected_data = array_mml<int>({0, 0, 0, 0, 0, 0, 0, 0, 0});
  auto actual_shape = t2->get_shape();
...
TEST(test_mml_tensor, test_index_1) {
  for (int i = 0; i < 100; i++) {
    array_mml<unsigned long int> shape = generate_random_array_mml_integral<unsigned long int>();
    const auto elements = accumulate(shape.begin(), shape.end(), 1UL, multiplies<unsigned long int>());
...
TEST(test_mml_tensor, test_indices_1) {
  for (int i = 0; i < 100; i++) {
    ...
    shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(shape, data);
    for (int j = 0; j < t1->get_size(); j++) {
      array_mml<unsigned long int> indices = array_mml<unsigned long int>(shape.size());
      int k = j;

      unsigned long int l = shape.size() - 1;
      do {
        indices[l] = k % shape[l];
        k /= shape[l];
      } while (l-- > 0);
...
TEST(test_mml_tensor, test_indices_2) {
  for (int i = 0; i < 100; i++) {
    ...
    auto indices = array_mml<unsigned long int>(shape.size());
    indices.fill(0);

    for (unsigned long int j = 0; j < t1->get_size(); j++) {
      ASSERT_EQ(data[j], (*t1)[indices]);

      unsigned long int k = shape.size() - 1;
      do {
...
TEST(test_mml_tensor, test_reshape_1) {
  for (int i = 0; i < 100; i++) {
    array_mml<unsigned long int> shape = generate_random_array_mml_integral<unsigned long int>();
    const unsigned long int elements = accumulate(shape.begin(), shape.end(), 1UL, multiplies<unsigned long int>());
    array_mml<int> data = generate_random_array_mml_integral<int>(elements, elements);
    shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(shape, data);
    t1->reshape({elements});
    auto expected_shape = array_mml<unsigned long int>({elements});
    auto actual_shape = t1->get_shape();
    ASSERT_EQ(expected_shape, actual_shape);
    auto expected_data = data;
    for (unsigned long int j = 0; i < elements; i++) {
...
TEST(test_mml_tensor, test_reshape_2) {
  for (int i = 0; i < 200; i++) {
    array_mml<unsigned long int> shape = generate_random_array_mml_integral<unsigned long int>(1, 1);
    const auto elements = accumulate(shape.begin(), shape.end(), 1, multiplies<unsigned long int>());
    array_mml<int> data = generate_random_array_mml_integral<int>(elements, elements);
    shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(shape, data);
    if (shape[0] % 2 == 0) {
      unsigned long int rows = shape[0] / 2;
      unsigned long int cols = 2;
      auto new_shape = array_mml<unsigned long int>({rows, cols});
