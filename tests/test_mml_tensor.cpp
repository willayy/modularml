...
TEST(test_mml_tensor, test_move_assignment_2) {
  shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(array_mml<uli>({3, 3}));
  shared_ptr<Tensor<int>> t2 = make_shared<Tensor_mml<int>>(array_mml<uli>({2, 2}));
  *t2 = move(*t1);
  auto expected_shape = array_mml<uli>({3, 3});
  auto expected_data = array_mml<int>({0, 0, 0, 0, 0, 0, 0, 0, 0});
...
TEST(test_mml_tensor, test_index_1) {
  for (int i = 0; i < 100; i++) {
    ...
    const auto elements = accumulate(shape.begin(), shape.end(), 1UL, multiplies<uli>());
...
TEST(test_mml_tensor, test_reshape_1) {
  for (int i = 0; i < 100; i++) {
    array_mml<uli> shape = generate_random_array_mml_integral<uli>();
    const uli elements = accumulate(shape.begin(), shape.end(), 1UL, multiplies<uli>());
    array_mml<int> data = generate_random_array_mml_integral<int>(elements, elements);
    shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(shape, data);
    t1->reshape({elements});
...
TEST(test_mml_tensor, test_reshape_2) {
  for (int i = 0; i < 200; i++) {
    ...
    const auto elements = accumulate(shape.begin(), shape.end(), 1, multiplies<uli>());
    array_mml<int> data = generate_random_array_mml_integral<int>(elements, elements);
    shared_ptr<Tensor<int>> t1 = make_shared<Tensor_mml<int>>(shape, data);
    if (shape[0] % 2 == 0) {
      uli rows = shape[0] / 2;
...
TEST(test_mml_tensor, test_slicing_1) {
  shared_ptr<Tensor<int>> t1 = tensor_mml_p({3, 3}, {1, 2, 3, 4, 5, 6, 7, 8, 9});
  shared_ptr<Tensor<int>> t2 = t1->slice({0});
  shared_ptr<Tensor<int>> t3 = t1->slice({1});
  shared_ptr<Tensor<int>> t4 = t1->slice({2});
  shared_ptr<Tensor<int>> expected_t2 = tensor_mml_p({3}, {1, 2, 3});
  shared_ptr<Tensor<int>> expected_t3 = tensor_mml_p({3}, {4, 5, 6});
  shared_ptr<Tensor<int>> expected_t4 = tensor_mml_p({3}, {7, 8, 9});
...
TEST(test_mml_tensor, test_slicing_2) {
  shared_ptr<Tensor<float>> t1 = tensor_mml_p({3, 3}, {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f});
  shared_ptr<Tensor<float>> t2 = t1->slice({0});
  shared_ptr<Tensor<float>> t3 = t1->slice({1});
  shared_ptr<Tensor<float>> t4 = t1->slice({2});
  shared_ptr<Tensor<float>> expected_t2 = tensor_mml_p({3}, {1.0f, 2.0f, 3.0f});
  shared_ptr<Tensor<float>> expected_t3 = tensor_mml_p({3}, {4.0f, 5.0f, 6.0f});
  shared_ptr<Tensor<float>> expected_t4 = tensor_mml_p({3}, {7.0f, 8.0f, 9.0f});
  ASSERT_EQ(*expected_t2, *t2);
  ASSERT_EQ(*expected_t3, *t3);
  ASSERT_EQ(*expected_t4, *t4);
}
...
TEST(test_mml_tensor, test_slicing_3) {
  ...
  shared_ptr<Tensor<int>> t5 = t1->slice({0, 0});
  shared_ptr<Tensor<int>> t6 = t1->slice({0, 1});
  shared_ptr<Tensor<int>> t7 = t1->slice({0, 2});
  ...
  shared_ptr<Tensor<int>> t8 = t1->slice({1, 0});
  shared_ptr<Tensor<int>> t9 = t1->slice({1, 1});
  shared_ptr<Tensor<int>> t10 = t1->slice({1, 2});
  ...
  shared_ptr<Tensor<int>> t11 = t1->slice({2, 0});
  shared_ptr<Tensor<int>> t12 = t1->slice({2, 1});
  shared_ptr<Tensor<int>> t13 = t1->slice({2, 2});
