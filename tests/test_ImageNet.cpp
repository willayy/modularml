 ...
 */
std::pair<size_t, size_t> imageNet(const size_t startingindex,
                                   const size_t endingindex) {
...
TEST(test_imageNet, imageNet_multithreaded) {
  ...
  const size_t images_per_thread =
      (total_images + num_threads - 1) / num_threads;  // ceiling division
  ...
  std::cout << "Total Success: " << total_success
            << ", Total Failure: " << total_failure << std::endl;

  // Assert a rough expected success rate (optional)
  float success_rate =
      static_cast<float>(total_success) / (total_success + total_failure);
  std::cout << "Success Rate: " << success_rate * 100 << "%" << std::endl;
